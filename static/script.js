  ...
  try {
    ...
    // reload history and chart for this user
    loadHistory();
    updateChart();
  ...
  recognition.onresult = async function(event) {
    ...
    try {
      const res = await fetch("/api/speech-mood", {  // ✅ FIXED URL
        method: "POST",
        headers: {"Content-Type": "application/json"},
  ...
  try {
    const res = await fetch("/api/risk-score", {   // ✅ FIXED
      method: "POST",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify({mood, sleep_hours:sleep, screen_time_hours:screen})
    });
  ...
  fetch("/api/history")
    ...
    .then(history => {
      if (history.length === 0) {
        chart = new Chart(ctx, {
          ...
          data: {
            labels: ["No data yet"],
            datasets: [{label: "Risk Score", data: [0], borderColor: "#007bff"}]
          }
        });
        return;
      }
      
      const labels = history.slice(0, 10).map(h => new Date(h.timestamp).toLocaleDateString());
    ...
    .catch(() => {
      chart = new Chart(ctx, {
        ...
        data: {
          labels: ["Demo"],
          datasets: [{label: "Demo", data: [50], borderColor: "#007bff"}]
        }
      });
    });
}
